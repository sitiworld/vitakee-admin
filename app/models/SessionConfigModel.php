...
class SessionConfigModel
{
    ...
    public function updateConfig($timeout, $birthdayMin)
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("UPDATE {$this->table} SET timeout_minutes = ?, birthday_min = ?, updated_at = ?, updated_by = ? ORDER BY config_id DESC LIMIT 1");
            if (!$stmt) {
                throw new mysqli_sql_exception("Error preparando el update: " . $this->db->error);
            }

            $stmt->bind_param("iisi", $timeout, $birthdayMin, $now, $userId);
            $stmt->execute();

            if ($stmt->affected_rows === 0) {
                // Insertar si no existe configuración previa
                $stmt = $this->db->prepare("INSERT INTO {$this->table} (timeout_minutes, birthday_min, updated_at, updated_by) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("iisi", $timeout, $birthdayMin, $now, $userId);
                $stmt->execute();
            }
