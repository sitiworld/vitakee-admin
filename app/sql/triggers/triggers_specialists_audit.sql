...
CREATE TRIGGER trg_specialists_update
...
BEGIN
  ...
  IF OLD.first_name <> NEW.first_name THEN
    ...
    INSERT INTO audit_log (
      ...
      'specialists', OLD.id, 'UPDATE', @user_id,
      change_data, NULL,
      @client_ip, @client_hostname, @user_agent,
      @client_os, @client_browser,
      @domain_name, @request_uri, @server_hostname
  ...
  IF OLD.specialty_id <> NEW.specialty_id THEN
    SET change_data = JSON_OBJECT(
      'specialty_id', JSON_OBJECT('old', OLD.specialty_id, 'new', NEW.specialty_id)
    );
    INSERT INTO audit_log (
      table_name, record_id, action_type, action_by,
      changes, full_row,
  ...
  IF OLD.title_id <> NEW.title_id THEN
    SET change_data = JSON_OBJECT(
      'title_id', JSON_OBJECT('old', OLD.title_id, 'new', NEW.title_id)
    );
    INSERT INTO audit_log (
      table_name, record_id, action_type, action_by,
      changes, full_row,
...
CREATE TRIGGER trg_specialists_delete
...
BEGIN
  INSERT INTO audit_log (
    ...
    'specialists', OLD.id, 'DELETE_PHYSICAL', @user_id,
    NULL, JSON_OBJECT(
...
CREATE TRIGGER trg_specialists_delete_logical
AFTER UPDATE ON specialists
FOR EACH ROW
BEGIN
  IF NEW.deleted_at IS NOT NULL AND OLD.deleted_at IS NULL THEN
    INSERT INTO audit_log (
      ...
      'specialists', OLD.id, 'DELETE_LOGICAL', @user_id,
