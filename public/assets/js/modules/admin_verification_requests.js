...
window.statusFormatter = (value, row) => {
  const statusMap = {
    PENDING: {
      class: 'bg-white-blue text-sapphire-blue border-sapphire-blue',
      text: traducciones['verification_status_pending'] || 'Pending',
    },
    AWAITING_PAYMENT: {
      class: 'bg-white-blue text-royal-blue border-royal-blue',
      text: traducciones['verification_payment_required'] || 'Awaiting Payment',
    },
    APPROVED: {
      class: 'bg-white-blue text-bright-turquoise border-bright-turquoise',
      text: traducciones['verification_status_approved'] || 'Approved',
    },
    REJECTED: {
      class: 'bg-neutral-light text-gray-dark border-gray',
      text: traducciones['verification_status_rejected'] || 'Rejected',
    },
  }
  ...
  $.ajax({
    ...
    error: () => {
      Swal.fire(
        ...
        traducciones['error_loading_requests'] ||
    ...
    success: (response) => {
      if (response.value && response.data) {
        ...
        Swal.fire(
          traducciones['error'] || 'Error',
          response.message ||
            traducciones['error_loading_request'] ||
...
$(document).on('click', '.approveRequestBtn', function () {
  ...
  Swal.fire({
    ...
    text:
      traducciones['confirm_approve_text'] ||
      'The specialist will be notified to complete payment.',
    icon: 'question',
...
$(document).on('click', '.rejectRequestBtn', function () {
  ...
  Swal.fire({
    ...
    text:
      traducciones['confirm_reject_text'] ||
      'The specialist will be notified of the rejection.',
    icon: 'warning',
    showCancelButton: true,
...
document.addEventListener('DOMContentLoaded', () => {
  // Expected format: /verification_requests_view?id=uuid-id-here
  const urlParams = new URLSearchParams(window.location.search);
  const requestId = urlParams.get('id');

  if (requestId && requestId.length > 20) {
    // We use a relative path just like in the click event to avoid BASE_URL issues
    $.ajax({
      url: `specialist-verification-requests/${requestId}`,
      type: 'GET',
      dataType: 'json',
      success: (response) => {
        if (response.value && response.data) {
          ...
          console.error('Could not load request from URL param', response.message);
        }
      },
      error: (xhr) => {
        console.error('Error loading request from URL param', xhr);
      },
    });
  }
});
